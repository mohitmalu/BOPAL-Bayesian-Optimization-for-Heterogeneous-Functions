...
from cbo_fin_datagen_v1 import rw_data
from cbo_fin_optfunc_rw import compare_TuRBO_rw
...
if __name__=='__main__':
   # Global Initialization
    rstate = 175  # Random State 123,130,175,
    prng = RandomState(rstate)
    verbose = True  # Verbose flag for printing
    dt = datetime.now().strftime("%m%d%y%H%M%S")
    N = 5  # number of data points exp - 1000, 2000, 5000
    d = 2  # dimension -  exp - 2, 4, 8
    d_par = 2  # No of dimensions with partitions
    obj_bounds = [(-1, 1)] * d  # bounds on the input data - same as space_bounds
    T = 200  # No of iterations of cbo and bo
    runs = 20  # Number of Macro reps
    ...
    tol = 0.025  # Tolerance for proposal distribution
    ...
    std = np.sqrt(0.0000)  # np.sqrt(0.001)  # Noisey evaluations
    ...
    exp_init = [(2,3,7,2,10)] # Single test case - par
    ...
    for i,j,ii,jj,a1 in exp_init:
        ...
        str1 = "exp_baselines/rw_all_parallel_"
        ...
        compare_TuRBO_rw(N, k, p, tup, obj_bounds, identifier, func_gen_ydata, d, T, freq, const, runs, y_opt, kernel_def,
